...
/**
 * Frame layout which contains a [WebtoonRecyclerView] (ported from yomi). It's needed to handle
 * touch events, because the recyclerview is scaled and its touch events are translated, which
 * breaks the detectors.
 */
class WebtoonFrame(context: Context) : FrameLayout(context) {
    ...
    /** Reused hit rect to avoid allocating on every touch event (hundreds per second). */
