...
- **Home's genre row offers every genre the app knows, anime included.** Home's "Browse by genre" used to carry only TMDB's film names, so the whole anime/manga vocabulary — Isekai, Reincarnation, Regression, Villainess, Otome Game, Harem, Reverse Harem, School Life, Slice of Life, Shounen, Shoujo, Seinen, Josei, Mecha, Mahou Shoujo, Martial Arts, Cultivation, Transmigration, Dungeon, Idol, Delinquents, Time Travel, Cooking, Gourmet, Webtoon, Manhwa, Manhua, Ecchi and the rest — was pickable in the Search tab and simply absent at home, and the television names were missing there too (Reality, Soap, Talk, Kids, News, Sci-Fi & Fantasy). Both strips now draw ONE list: TMDB's film names, TMDB's television names, and 99 anime/manga tags — so the two screens cannot disagree again, and Search gains nothing to lose by sharing it. A name TMDB has a genre id for opens the grid for films AND series at once (both namespaces' ids are sent, and TMDB answers each namespace with the one it knows — verified against the live API), and an anime tag TMDB has no genre id for opens the grid for its TMDB keyword instead, since that is the only filter TMDB can apply to it. Every tag's keyword was checked against the live API before shipping: 97 of 99 resolve, and every one of those answers titles in at least one namespace; the two that had no keyword of their own ("Guilds", "Crossdressing") now point at the keyword TMDB actually uses ("adventurer", "cross dressing" — TMDB spells it with a space). A tag whose keyword resolves to nothing no longer silently drops the filter and answers with the whole popular catalogue: it shows an empty page, which is honest, where a page of unrelated titles was not.

### Fixed

- **The Extensions tab no longer stutters, and a fast scroll no longer drags the whole app down with it.** Three separate things were being redone on the draw path, and they compounded. First, the "which repository is this extension from?" label was answered by scanning the entire repo list for each of the ~500 installed providers, and re-parsing each repo's URL inside that scan — hundreds of thousands of regexes, run the moment the list was drawn; it is now answered from a repo index built once per change, so a lookup is a hash map hit. Second, the update check (it hashes every installed extension file) and the 18+ adoption pass ran again on EVERY repo listing arrival — and the listings arrive one repo at a time, so a load re-hashed the whole library dozens of times while the user was scrolling it; both are now coalesced in the ViewModel to run once, after the arrivals settle. Third, every row re-probed its icon whenever it scrolled back into view — a plugin load for a CloudStream one, an extension load for an Aniyomi/manga one — and the gear's "does this plugin have a settings screen?" probe loaded the plugin just to answer; the settings probe now consults only the already-loaded cache (tapping the gear still loads it on demand), a provider with no icon is remembered as having none for a few minutes instead of being re-probed, and an icon is resolved only after its row has stayed on screen for a moment, so a fling never triggers a probe for the rows it flies past. The URL-identity parse (the branch / jsDelivr / `github.com` blob spellings of one file) is memoised as well, which is on the same path. Net effect: scrolling the installed list is drawing work only — no file hashing, no plugin loading, no URL parsing.
- **Opening the provider picker on Home is instant.** The sheet labels each row with the repository its extension came from, and that map used to be computed the moment the pill was tapped — over the same ~500-provider × repo-list match described above, which is the second of delay between the tap and the sheet. Home now computes the map while idle (remembered on the provider and repo lists) and hands it to the sheet, so a tap only opens the sheet; the Search tab's own picker keeps computing it locally, from the same code.
- **A manga catalogue, search or chapter list that needs a browser now loads one itself, so the globe button is no longer the thing that makes a chapter list appear.** The reported shape was a manga extension whose catalogue (and chapter list) came up empty — with no verification prompt offered — until the site was opened once in the WebView by hand and closed again. That is the signature of a site that wants a real browser session: the extension's own client only clears a challenge it can RECOGNISE, and a source that answers a refused request with an empty 200 (or builds its own HTTP stack so the app's interceptor never sees the challenge) leaves nothing to recognise — while the globe DID work, because that WebView earned the site's cookies and the next request went through with them. The manga engine now does that itself: a catalogue (page one), a search or a chapter list that comes back empty or fails loads the site ONCE in the offscreen WebView, spaced per host so it cannot become a page load per request, and asks again with the session it earned — nothing opens on the user's screen. A retry that also fails reports the retry's reason, not the first failure's. And a real bot wall (403/429/503, Cloudflare, "just a moment") is now recorded as one, the way the anime half has always done it, so the walled site is named to the UI and left out of a cross-extension search instead of quietly answering nothing on every lookup.

## 0.10.49

### Added

- **Vega is a source kind of its own — the whole Vega provider ecosystem installs and plays here now.** [Vega](https://github.com/Zenda-Cross/vega-providers) publishes its providers as a **bare JSON array** at `manifest.json` — not a `repo.json` with a `plugins` key — and each entry names a provider whose code is a folder of CommonJS modules under `dist/<value>/{catalog,posts,meta,stream,episodes,settings}.js`. Those files expect a `providerContext` holding axios, cheerio, the repo's shared desktop-Chrome headers, a per-provider key/value store and a bot-wall solver, none of which is a file you can download: a Vega provider is a *program*, not an extension archive. Hikari now runs it as one. The repo is registered on first run (and `vega`, `vegarepo`, `zendacross` all resolve to it in "Add repository"), "Vega repos" is a row in Sources beside CloudStream/Hikari/Nuvio/SkyStream/Aniyomi, and a Vega provider is a full citizen of the app: its `catalog`/`genres` lists become Home rows and catalogues, `getSearchPosts` answers search, `getMeta` fills the detail page, `linkList`/`getEpisodes` build the episode list and `getStream` supplies the servers. The runtime recreates the provider's world honestly — an axios over the same OkHttp bridge nuvio uses (including `validateStatus`, `params`, `maxRedirects: 0` and `responseType: 'arraybuffer'`, so a provider that branches on `error.response.status === 403` behaves exactly as it does upstream), the real cheerio bundle, the repo's own `headers.js`, and a `kvStore` persisted to the provider's folder — and it loads **one module per call** (the published files are self-contained bundles, with no cross-file `require` anywhere in `dist`) compiled once to QuickJS bytecode and reused. A provider that exports no catalogue (they exist — a search-only scraper) says so instead of failing, and uninstalling removes its folder.
- **A genre strip on Home, under the banner.** Choosing a genre from the row starts browsing everything tagged with it — films, series and anything else a source said carries that tag — so a genre is something you can *pick* rather than something you have to type into a search. It is the same genre list (and the same multi-select behaviour) as the Search tab's strip, so the names match by construction.
- **Every installed extension now says which repository it came from.** Four "AniKoto" rows labelled only "Cs3" told nobody which repo each one was: the engine is what the app knows, and the repo is what the user needs. The installed list, the extension picker on Home and the "Choose an extension" sheet now draw `AniKoto · Hikari · CNC Verse` instead of `AniKoto · Hikari`, and the rows of a repo's listing carry their repo's name too, so a search that flattens the groups still says where each entry lives. The match is made through the URL's GitHub `owner/repo` (see `RepoProvenance`) because the install remembers the extension *file* while the repo list remembers the repo's *index* — two strings that never agree literally — and an extension installed by hand or from a since-removed repo simply shows its engine, as before.
- **The Home search box searches on Home.** Tapping the magnifier beside the globe used to hand you to the Search tab; it now opens the box in place, over whatever is on screen, and searches with the provider Home is currently loading its catalogue from — the same scope the catalogue behind it uses. Its results are the provider's own, debounced, with the engine's own failure line under an empty result, and Back closes the box rather than navigating.

### Changed

- **The catalogue screen is laid out the way Nuvio lays one out.** Three columns on a phone (the old adaptive 84dp grid gave four cramped ones), 16dp gutters and a real gap between rows so every tile is its own card, the list's own name drawn at heading size with the engine under it rather than squeezed into a toolbar row, the title under each poster plus a second, quieter line (year and first genre) the way Nuvio prints the release date, and a skeleton grid while the first page arrives instead of a lone spinner. The column count grows by screen width — four, five, six, seven — and on a television it is still the living-room cell size that decides it, not this number. Corner rounding and "Show titles" still come from Settings → App Layout, so a user who asked for label-free artwork keeps it.

### Fixed

- **The bundled YTS extension is gone.** "YTS" shipped as a demo Hikari extension and is not needed: it never answered with a catalogue or anything to browse — tapping it showed nothing at all — so it was a row that could only ever waste a tap. Its provider class is removed and a first launch after this update sweeps the row once, leaving every other extension alone.
- **Aniyomi/Mihon repos that publish only a protobuf index now work.** The extension list a Mihon repo serves is not one fixed file name, and the important discovery is that there are now three shapes in the wild: the official Aniyomi repo publishes `index.min.json`, keiyoushi publishes `index.json` (with `index.min.json` reduced to a two-entry "update your app" placeholder), and repos that have moved to Mihon 0.20.1+ publish `index.pb` — **protobuf, no JSON at all**. A `.pb` index is bytes, so it takes its own fetch and decode (gzipped, since the published file is `1f 8b`); the decoder reads the same fields Mihon's own extension store reads and answers in the JSON shape every other index uses, so nothing downstream changes. Decoding was verified against the real keiyoushi `index.pb` (1377 entries, store name "Keiyoushi", 365 NSFW-tagged, no empty APK URLs), and every candidate name is still tried in order, with a placeholder or empty index skipped rather than installed from.

## 0.10.48

### Fixed

- **Manga chapters load again — the chapter list is now asked for the way a modern extension actually answers it, so "chapters failed: HTTP error 404" is gone.** Reading a series from a source like Comix showed its artwork and its name and then no chapters at all, with a red `HTTP error 404` line where the list should be — while the same source listed its chapters perfectly in another app. The cause was the *call*, not the source: Hikari asked for details and chapters through `getMangaDetails`/`getChapterList`, which are the old extensions-lib 1.5 entry points. A source built against extensions-lib 1.6 (every source in the keiyoushi ecosystem now, Comix included) does not implement those at all — it implements one combined call, `getMangaUpdate(manga, chapters, fetchDetails, fetchChapters)`, and leaves the two old ones to the base class, whose request is built as "the source's base URL plus the manga's url". Comix's catalogue hands out `/12345-slug` (the extension itself turns that into `comix.to/title/12345-slug`), so the old call asked for `comix.to/12345-slug` — a page that does not exist, hence a 404 — and its chapter list is in any case an API reply the extension can only sign inside its own combined call. Details and chapters now go through that combined call, exactly as the app's anime half has always read episode lists (`getAnimeEpisodeUpdate`) and as the reference reader drives every manga extension, with one flag per call so each screen costs exactly the requests it used to. A source that predates the combined call is unaffected: the app's own base class bridges the combined call straight back to the two old ones.
- **A television remote can pick from every list again — starting with the app language.** On a TV, opening Settings → App language (or any other "pick one" panel) and pressing the D-pad did nothing useful: the panel's dimmed backdrop and its tap-catching card are both tappable, and a tappable thing is also something the remote can land on — so the first press of the D-pad put the cursor on the full-screen backdrop, and centre press ran the backdrop's "close" action. The picker shut instead of picking. Those two containers no longer take the remote's cursor (touch is untouched — tapping outside still closes the panel), and a picker now places the cursor on its first row as it opens, so the very first arrow press walks the list of languages rather than the panel's own chrome. This is one shared panel, so it fixes the app language, the fonts, the TMDB language, the DNS resolver, the loading-effect and poster pickers and every other choice list at once.
- **A television no longer inherits a phone's video-enhance preset, which is what made the picture stutter on a TV stick.** The video-enhance presets are real GPU colour grades — the picture itself is processed, which is the point — but that costs a graphics pass on every single decoded frame, and on a TV stick, which is already busy decoding, that pass is visible as a video that stutters. It costs nothing on the default preset (Natural applies nothing at all), so this only ever hurt a television whose preset had arrived from somewhere else: the choice travelled with the account from a paired phone. The preset is now a per-device setting like the TV layout and the performance mode are, so one device's picture processing cannot be put on another, and a TV that has not chosen a preset for itself is put back on Natural once. The "this device cannot run video effects" note is per-device for the same reason: a phone whose graphics stack refused the pipeline must not tell a television that it cannot run one.

### Added

- **Every anime genre is in the Search tab's genre strip, not just the film and television ones.** Isekai, School Life, Slice of Life, Shounen, Shoujo, Seinen, Josei, Mecha, Harem, Reverse Harem, Ecchi, Mahou Shoujo, Martial Arts, Super Power, Samurai, Idol, Delinquents, Reincarnation, Time Travel, Cooking, Gore, Survival, Parody, Kids, Music, Sports, Supernatural, Psychological, Military, Mythology, Historical, Vampire, Demons, Space, Game, Magic, Tragedy, Racing, Detective, Police, Gender Bender, Boys Love, Girls Love, Iyashikei, Gourmet, Award Winning, Workplace, Villainess and Crossdressing are offered beside the TMDB names (which already carried Romance, Action, Comedy and the rest), matched case-insensitively against what a source said about itself — so an anime/manga source's own tags are now a filter the strip can actually narrow to.
- **Two more Nuvio bundles, and they come with the others.** Typing `nuvio` into "Add repository" adds the whole set of bundled Nuvio provider repos at once; **Anime-Nuvio** (AllAnime, AnimeKai, AnimePahe, Anime-Sama, AnimeWorld and the rest of the anime sources) and **Hindi-Nuvio** (4KHDHub, VegaMovies, HDMovie2, CineStream, FlixIndia and the rest of the Hindi/Indian sources) are now part of that set and part of the first-run seeding, so both are in the Nuvio folder of the Extensions tab on a fresh install. They also answer to their own short names (`animenuvio`, `hindinuvio`, `hindi`).

### Changed

- **Hikari no longer installs any Nuvio provider for you — only the repos.** A fresh install used to arrive with three providers already installed (vixsrc, moviebox, showbox), which is a choice the app should not make on the user's behalf: the repos are still seeded, so every one of those scrapers is one tap away in the Extensions tab, but which are installed is the user's to decide. An install that already has the pre-installed ones has them removed once — and only those, matched on the exact file inside the repo they came from, so a provider of the same name the user installed themselves (MovieBox also ships in the Hindi bundle) is left alone.

## 0.10.47

### Added

- **The Telegram tab's three sections swipe.** "My chats", "Added channels" and "Video links" are pages of one screen, so they now answer the gesture every other page of this app answers — swipe left and right — exactly as My Stuff's sections do. The pill the user taps and the page a swipe settles on are the same state in both directions: tapping a pill animates the pager over (so the move reads as a page turning), and a swipe that lands on a section lights that pill up.
- **A genre strip on the Search tab, and Anime beside Movies and Series.** The genre strip is a scrollable row of every genre there is — the union of TMDB's film and television genres, which are the names a TMDB-sourced catalogue carries and the names extension tags most often match — and it is multi-select ("action, comedy" keeps both). The list is fixed rather than built from the results, so a genre can be picked before the search has found anything. Choosing one keeps only results whose source said that genre, and the count of results hidden because their source said no genre at all is shown next to "N shown · M found", so the strip can never look like it did nothing. **Anime** is the fourth kind chip: no provider has an "anime" media type, so it counts a result as anime when the addon's own type string says so (an anime catalogue is the type `anime`), when a genre is "Anime", or when it is TMDB's "Animation" — which is the genre every TMDB-sourced anime title carries.
- **The Search tab remembers what you searched for.** The queries that actually ran are kept, newest first, under a "Recent searches" heading above the empty state — shown while the search box is empty, so results are never pushed down by a list of old queries. Tapping a chip re-runs that search; the ✕ on a chip forgets just that one; "Clear all" forgets the lot. Searching for the same thing again moves its entry to the top rather than adding a second copy, the list is capped at twenty, and a one-letter box is never recorded (it is a search the user has not finished typing).

### Fixed

- **Back out of a settings sub-folder no longer throws you to the top of the folder you came from.** The index, an open folder and a sub-folder were sharing scroll positions — so the page that came back had to be scrolled to its own top on every step (a page opening half-way down looks broken), which is what also threw the page you returned to up to the top. The report was Settings → App Layout → Poster styling, then Back, and landing at the top of App Layout instead of on the row that was tapped. Each page now keeps its own scroll position and only a page you are *opening* is placed at its top — "back" simply leaves the page you return to exactly where you left it, at every depth.
- **A television remote can now land on the rows of the Extensions tab, and the remote's highlight shows which row it is on.** Rows were plain containers: the D-pad's focus search skipped straight past them and could only ever stop on the small button at the far right end of a line, so on a TV it was not possible to see which option the remote was on — the reported "I press the button on the remote and nothing on screen shows where I am". Every row of a repo (the extension entries you install from), every installed extension and every website in Sources is a focus target now, wearing the same focus ring as the rest of the app, and pressing one does what the row's own button says — install, update or uninstall in a repo; the switch on an installed extension; open for a website. A row's press answers *after* its own buttons have had the key, so pressing Uninstall uninstalls and only a press nothing else claimed belongs to the row.

## 0.10.46

### Fixed

- **The Telegram tab's "Video links" section was off the screen, so that half of the tab looked like it did not exist.** The row of sections was laid out with the screen's 16dp edge inset as *layout* padding, which made the strip 32dp narrower than the screen, and once the two section pills carried their counts the third ("Video links") sat entirely past the right edge — on a row with no scrollbar, nothing said it was there. The strip now spans the full width with that inset as *content* padding, so the next pill runs under the screen edge and stays genuinely visible at rest, and the pills are a step more compact (a smaller icon, the type scale down a step, tighter padding) so a good slice of it shows. Swiping to the section always worked; the point is that you can now see there is one.
- **Adding a channel or a video link now leaves you on the section you added to.** A public channel put you back on "My chats", so the row you had just created was in a section you then had to go and find; a video link went to a section most users had not yet seen at all, which read as the add having done nothing. The add now switches to "Added channels" or "Video links" respectively, with the new row in front of you.
- **Back out of a Telegram chat, channel or section no longer left the tab.** The Telegram tab's screens are navigation destinations and nothing there answered the back button, so the press fell through to the app and backing out of a chat landed on Home. Back now steps out of what is open — an open chat, then an open channel or video, then a section other than "My chats" — and only the next press leaves the tab.
- **Back out of a settings folder no longer jumps the folder list to the top.** The index and an open folder shared one scroll state, and a folder had to be forced back to its own top on every switch (a folder opening half-way down would look broken) — which also threw the index to the top, so leaving Settings → Playback put you at the top of Settings instead of on the row you opened it from. The two pages keep a scroll state each now: a folder still opens at its top, and the index is left exactly where you were.
- **The explanation paragraph on Profiles kept showing with "Explanations" switched off.** It was drawn outside the sheet's own list, where the setting is not in scope, so it was the one line in the app that ignored the switch. It obeys it now like every other explanation.
- **A television remote could not select a provider on Home, so a catalog could not be loaded.** The "Choose an extension" rows handle their own taps — a tap opens a provider, a half-second hold multi-selects — through a raw pointer gesture, and a raw pointer gesture is invisible to Compose's focus system: the D-pad walked straight past the whole list and a centre press did nothing at all. Those rows now also carry a focus target and a centre-press handler (`Modifier.tvPress`, applied alongside the gesture rather than instead of it, so touch still behaves exactly as before): the remote can walk the list, land on a row with the same highlight a focused button gets, and open it. The manga engines on the Read tab had the very same defect and the same fix.
- The "Playback start" radio choices in Settings were the same kind of unreachable target — a small radio button alone on a full-width row — and the row itself now takes the press; the search box's translate button, the reader's auto-scroll-speed slider and the reader's page scrubber (one page per press) now answer the D-pad too.

### Added

- **The player's "Load from internet" panel can be narrowed to one subtitle language.** Searching a title brought back every language the sites held at once — an English release, three Portuguese, a Hindi, and whatever else — in one list, so the track you wanted sat somewhere in the middle of fifty rows. A scrollable bar now sits above the title box: All (the default), your own language, then the ones this app is asked for most — English, Hindi, Arabic, French, Spanish, Portuguese, Indonesian, Bengali, Tamil, Telugu, Malayalam, Urdu, Russian, Chinese, Japanese, Korean. Picking one shows only that language's tracks; the two spellings a site may use ("Arabic" and "ara") count as the same language; the per-source counts beside the status line follow the choice; and the pick is remembered for next time. Changing it re-filters what is already on the screen instead of searching the sites again, so the list answers instantly rather than after a second wait. The sites are still asked in full — none of them narrows by locale, so the filter is applied here — but the request now carries the chosen language, which the sites that order by language use to put it first.
- The "Hold any provider for half a second to select more than one." line now stays put even with explanations switched off: it is the only place the gesture is written down, and hiding it made multi-select undiscoverable exactly for the user who had tidied the interface.
