...
Test build — the extension you opened a title FROM is now the extension that plays, the "N still searching" counter can no longer freeze on a stuck repo, junk pages that echo the search term back in their title are no longer accepted as matches, and no loading screen is flat black any more.

**Your own extension's server is now the server that plays.** Reported, and this is the same complaint in a third form: "I am playing from hikari extension mrds, and on that extension all video is in chinese so it only has two server and both on mrds extension, none other extension has these video, still see it selected xfree and played wrong video and didn't even shows up the real server the mrds server for it, on second time it shows the mrds server." Nothing in that report is a mystery any more, because it was three separate things stacking up, and all three are fixed here.

*First, the order of the list was pure arrival order.* The player plays the row its start-index rule picks and walks the list top-down when a server dies, so **whichever repo answered fastest won the top row** — and for a title that lives on one Chinese .hiki repo, an adult tube repo that answers in a second with a same-named video got the top row and got played, while the origin's own link (a cold plugin, still spinning up its runtime and its site session) was not even on the list yet. The servers the user's own extension produced are now placed **first**, always, ahead of everything else, and only then does the remembered/last-used server and arrival order follow. Tapping a movie inside an extension now plays that extension's link, which is what the tap meant.

*Second, the wait for the origin was too short to ever be useful.* The player deliberately holds off starting on another extension's server for a moment while the origin answers — but that hold was **1.2 seconds**, chosen back when it was thought a repository answers in under a second. A .hiki plugin's first call has to boot its runtime and open the site's session; 1.2s is not enough for it to be *asked*, let alone answer, so the hold expired every single time and the player fell through to whatever else had answered. The hold is now **45 seconds**, and — more importantly — it does not normally run out at all: the pass tells the player the moment the origin has **answered** (with servers, or with "nothing for this episode"), and that signal, not the clock, is what ends the wait. An extension that plainly has nothing for the episode no longer costs the user the whole backstop, and one that needs twenty seconds to answer is no longer cut off at two. If the grace window does expire with nothing from the origin, the log now says so in as many words (`origin "MRDS" (…) never answered in the 45000ms grace window — starting on the first server from anywhere`) instead of the user having to guess why a different extension's link played.

*Third, the origin's one cold call was thrown away when it timed out — and never repeated.* The origin was asked exactly once, with the same 45s budget as everyone else, and that single call is the one that pays the plugin's cold start. If it timed out, threw, or was cancelled mid-flight (which is the normal end of a pass — the detail screen is re-created when you back out of the player, and its scope dies with the pass), the pass simply moved on: the origin had produced nothing for a title that lives on it. The origin now gets a **short 12-second probe first**, then the full budget, and it is always given at least two attempts rather than the provider's normal one; and if the pass still ends without a single server from it, and the origin never plainly said "I have nothing for this episode", it is **asked once more in the background** with the plugin warm, and whatever it finds is pushed into the same live list the player is already showing. That is the mechanism behind "playback failed … and on the second attempt after taking 70 second it played the video on mrds server": the first pass gave up on the origin's cold call, and the second attempt found it warm.

**"It stuck on 16 still searching and didn't go further."** Reported: "see in one extension it stuck on 16 still searching and not gone further i have to go back and play again to load all server." That number is the pass's live count of repos still working, and it could only ever go down when a repo's own call returned — so a plugin that wedged (its runtime hung, its socket never answered) held its place in that count and in a concurrency slot **forever**, and the line sat frozen at 16 while the pass waited on calls that were never coming back. There is a watchdog for exactly this, and it was returning the slot but **leaving the entry in the live progress map**, so the count never moved. The watchdog now also takes the repo off the progress line and records it as "stopped responding", the count moves, and the reason is in the log (`extension '…' has not answered in 90s — its slot has been given back, it is off the progress line, and it is skipped for this session`). The ceilings are per-kind, because the budgets genuinely differ: a search that has not answered in 90 seconds is stuck (its own budget is about 20), while an Aniyomi extraction or a detail lookup is allowed 120 seconds and 200 respectively, since those legitimately are slow.

**A page title that echoes the search back is not a match.** While tracing the wrong-video report: several adult-tube repos hand over the *request* as the row's title — `…&query=<what you typed>` — so the "title" contains the very words the user searched for and therefore passes every word-containment rule the matcher has, scoring as the best match on the page. Titles that are really addresses (they contain `http://`, `https://`, `www.`, or a `?query=`/`&q=`/`&amp;search=`-style parameter) are now rejected outright before scoring, so an echo page can no longer be chosen and played.

**No loading screen is flat black any more.** "Also see in some see the loading screen showing black if its issue then fix it." It was two separate blacks. On the detail page, the full-screen title card's background was a hard `#000000`, and its scrim was drawn at full strength over it — so a title whose provider sends **no artwork at all** (the normal case for these scraped .hiki titles) spent the entire server search on a black rectangle with a title on it, which is indistinguishable from a crashed app. The card now draws a deep wash in the app's own surface tones with a soft bloom behind the title whenever there is no art to show, and the scrim thins itself out in exactly that case instead of re-blackening what was just lifted. In the player, the same cover's background was also `#000000`, and the same three loading styles that show no artwork (Minimal, and Cinematic/Poster for a title with no poster) were black for the whole wait; they now sit on the same wash, and a plain-gradient fallback drawable is the cover's background in every style. With artwork present nothing changes — the art and its scrim still cover it exactly as before.
